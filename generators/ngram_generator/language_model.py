...
#     train_path = data_dir.joinpath('train.txt').absolute().as_posix()
    train_path = data_dir.absolute().as_posix()
    test_path  = data_dir.absolute().as_posix()
...
class LanguageModel(object):
    ...
    def _smooth(self):
        ...
        def smoothed_count(n_gram, n_count):
            ...
            '''CHANGE INTO CONDITIONAL P'''
            return (n_count/len(list(n_grams)))/(m_count/len(list(m_grams)))
    ...
    def _best_candidate(self, prev, i, without=[]):
        ...
        mgram_list = [x[0] for x in candidates]
        mgram_prob_list = [y[1] for y in candidates]
        print(sum(mgram_prob_list))
        if len(candidates) == 0:
            return ("</s>", 1)
        else:
            if len(prev)>=1:
    ...
    def generate_sentences(self, num, min_len, max_len):
        ...
        for i in range(num):
            ...
            yield ' '.join(sent), prob#-1/math.log(prob)
